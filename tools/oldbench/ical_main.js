...
function updateTimezones(vcal) {
  ...
  for (i in reqTzid) {
    if (reqTzid.hasOwnProperty(i) && !vtimezones[i] && TimezoneService.has(i)) {
...
function dumpn() {
  ...
  if (typeof console !== 'undefined' && 'log' in console) {
    ...
    dumpn = function consoleDumpn(input) {
      // eslint-disable-line no-func-assign
      console.log(input); // eslint-disable-line no-undef
    };
  } else {
    dumpn = function geckoDumpn(input) {
      // eslint-disable-line no-func-assign
      dump(input + '\n'); // eslint-disable-line no-undef
    };
  }
...
function inherits(base, child, extra) {
  function F() {}
    ...
    value: function expand_year_days(aYear) {
      ...
      if (partCount == 0) {
        ...
        for (var monthkey in this.by_data.BYMONTH) {
          ...
          var daysInMonth = Time.daysInMonth(month_, aYear);
        ...
        for (var monthkey in this.by_data.BYMONTH) {
          ...
          var daysInMonth = Time.daysInMonth(month, aYear);
          t.year = aYear;
        ...
        for (var daykey in expandedDays) {
          ...
          var tt = Time.fromDayOfYear(day, aYear);
  ...
  _createClass(RecurExpansion, [{
    ...
    value: function _extractDates(component, propertyName) {
      function handleProp(prop) {
        idx = binsearchInsert(result, prop, function (a, b) {
          return a.compare(b);
        }); // ordered insert
