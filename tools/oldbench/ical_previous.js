...
const OPTIONS = ["tzid", "location", "tznames",
               "latitude", "longitude"];
...
function dumpn() {
  ...
  if (typeof (console) !== 'undefined' && 'log' in console) { // eslint-disable-line no-undef
    dumpn = function consoleDumpn(input) { // eslint-disable-line no-func-assign
      console.log(input); // eslint-disable-line no-undef
    };
  } else {
    dumpn = function geckoDumpn(input) { // eslint-disable-line no-func-assign
...
class VCardTime extends Time {
  ...
  toString() {
    ...
    var datepart = (hasYear ? pad2(y) + (hasMonth || hasDay ? '-' : '') : (hasMonth || hasDay ? '--' : '')) +
                   (hasMonth ? pad2(m) : '') +
                   (hasDay ? '-' + pad2(d) : '');
    var timepart = (hasHour ? pad2(h) : '-') + (hasHour && hasMinute ? ':' : '') +
...
class RecurIterator {
  ...
  next_generic(aRuleType, aInterval, aDateAttr,
